...
if(isset($_POST['submit'])){
    ...
    $check1 = "SELECT * FROM `tbl_users` WHERE `useremail`='$email'";
    ...
    if($rsltcheck == 0){
        ...
        if($reg_query){
             ...
             $ws_reg = "INSERT INTO `tbl_workshop`(`users_id`, `ws_name`,`username`, `place`, `address`, `email`, `phone`, `status`) VALUES ('$users_id','$wsname','$username','$place','$address','$email','$phone','$status')";
            $user_reg_query = mysqli_query($conn,$ws_reg);
            ...
            echo'<script>window.location.href="add_workshop.php";</script>'; 
        }
