...
//echo $email; exit;
//echo $pass; exit;
$q7="select mech_name,ws_name,place,address,phone from tbl_mechanic where username='".$n."'";

$result7 = mysqli_query($conn,$q7);
//echo $result;
$row2 = mysqli_fetch_array($result7);
$mech_name = $row2['mech_name'];
$place = $row2['place'];
$address = $row2['address'];
$ws_name = $row2['ws_name'];
$phone = $row2['phone'];
...
//   $address=$_POST['address'];
  ...
  $sql2="update `tbl_mechanic` set mech_name='$mech_name',place='$place', phone='$phone' where username='".$n."'";
  $result3 = mysqli_query($conn,$sql2);
  if($result2)
  {
    $update=true;
    echo'<script> alert ("Account Updated");</script>';
    echo'<script>window.location.href="view_mech.php";</script>';
  }else{
    echo'<script> alert ("username  already exists!");</script>';
    echo'<script>window.location.href="update_mech.php";</script>'; 
}
...
//email
                                ...
                                function Validateemail()
                                        ...
                                        if(!email.match(/^[\w\+\'\.-]+@[\w\'\.-]+\.[a-zA-Z]{2,}$/)){  
                                            document.getElementById('email1').innerHTML="Please enter ";  
                                            document.getElementById('email').value = email;
                                                document.getElementById('email').style.color = "red";
                                        return false;  
                                        }else{
                            ...
                            <form class="user" method="post" action="" onsubmit="return Val();">
                                <div class="form-group">
                                    <!-- <div class="col-sm-6 mb-3 mb-sm-0"> -->
                                        <input type="text" class="form-control form-control-user" id="name" name="mech_name" value="<?php echo "$mech_name"; ?>"
                                            placeholder="Mechcanic Name" required>
                                    </div>
                                ...
                                <div class="form-group">
                                    <input type="number" class="form-control form-control-user" id="phone" name="phone" value="<?php echo "$phone"; ?>"
                                        placeholder="Phone" onkeyup="return Validphone()"/>
                                </div>
                                <span id="msg2" style="color:red;"></span>
                                <div class="form-group">
                                        <input type="password" class="form-control form-control-user" name="password" value="<?php echo "$pass"; ?>"
